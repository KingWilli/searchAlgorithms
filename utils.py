...
class PriorityQueue(Queue):
    """Queue where items are sorted by lt"""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=sort_by_weight, reverse=True)

    def pop(self):
        return self.A.pop()


class HeuristicsPriorityQueue(PriorityQueue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda e: sort_by_underestimation(e, self.problem), reverse=True)
...
def sort_by_underestimation(e, problem):
    return e.path_cost + problem.h(e)
